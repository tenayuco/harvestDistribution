...
plotStates(modelos$C, animals ="C_1", ask = F)
```
...
plotStates(modelos$O, animals ="O_1", ask = F)
```

\newpage

In the following we plot two examples of the results of the Viterbi algoritm for C_1 and C_2, O_1 and O_2 (see main trajectories in main text.)

```{r echo=FALSE, fig.cap="Distribution of states $St$ along two trajectories. Each color depict one of the states (orange: 1, blue: 2)", fig.keep=c(3,4), fig.show='hold', message=FALSE, out.width="50%"}
...
plot(modelos$O, ask = F, plotTracks = TRUE, breaks = 100, col = groupColors2, lwd=3.0)
```
