...
dataCosecha <- dataCosecha %>%
  separate(ID, into= c("Farm", "IDperFarm"), remove= FALSE)
...
for (f in modes){
  dataCosecha_F <- dataCosecha %>% filter (Farm == as.character(f))
  
dataCosecha_F$Farm <- NULL
dataCosecha_F$IDperFarm<- NULL

dataCosecha_F <- prepData(dataCosecha_F, type= "UTM", coordNames = c("x", "y"))
...
for (modelStep in c("weibull", "gamma")){
  ...
  #op1
    tryCatch({
    m_cosecha<- fitHMM(data = dataCosecha_F, stepDist = modelStep,  
...
write_csv(DF_TOTAL, paste("../output/wholeTable_100reps_",as.character(f), ".csv", sep = "_"))
}
