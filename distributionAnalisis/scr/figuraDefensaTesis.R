...
#DF_HARVEST_GAMMA_E <- read.csv("/home/emilio/archivosTrabajandose/harvestDistribution/distributionAnalisis/data/analyzedData_figuresFINCA_O_.csv")
...
DF_HARVEST_GAMMA$state[DF_HARVEST_GAMMA$state == "1"] <- "Cosecha"
DF_HARVEST_GAMMA$state[DF_HARVEST_GAMMA$state == "2"] <- "Búsqueda"
...
DF_HARVEST_RESUMEN <- DF_HARVEST_GAMMA %>%
  ...
  filter(farm_ID %in% c("O_1", "O_2", "C_3", "C_4"))

#we plot the states. It is important to notice that states do not represent the same
FIG_MAP_GAMMA_SAM<- DF_HARVEST_GAMMA %>% 
  unite("farm_ID", c(farm, IDREC)) %>% 
  filter(farm_ID %in% c("O_1", "O_2", "C_3", "C_4"))%>% 
  ggplot(aes(x= x, y = y)) +
  geom_path(aes(col= as.factor(state), group = (farm_ID)), size= 1)+
  geom_point(size=1, aes(fill= "Árbol visitado"))+ # es importante que sea path, porque así lo hace según coo estan ordenados los
  scale_color_manual(values= mycolsStates)+
  facet_wrap(~farm_ID, ncol = 2)+
  ...
  labs(x= "X (m)", y= "Y (m)", col= "Estado", fill= "")
...
ggsave(FIG_MAP_GAMMA_SAM, filename= "../output/figurasDefensa/sam_mapHarvest_porFarm.png", height = 9, width = 9, device = "png")


FIG_MAP_GAMMA_SAM_GRAY<- DF_HARVEST_GAMMA %>% 
  unite("farm_ID", c(farm, IDREC)) %>% 
  filter(farm_ID %in% c("O_1", "O_2", "C_3", "C_4"))%>% 
  ggplot(aes(x= x, y = y)) +
  geom_path(aes(group = (farm_ID)), col= "darkgray", size= 1)+
  geom_point(size=1, aes(fill= "Árbol visitado"))+ # es importante que sea path, porque así lo hace según coo estan ordenados los
  scale_color_manual(values= mycolsStates)+
  facet_wrap(~farm_ID, ncol = 2)+
  geom_text(x = 140, y = 10, aes(label = observation), data = DF_HARVEST_RESUMEN)+
...
#ggsave(FIG_MAP_GAMMA, filename= "/home/emilio/archivosTrabajandose/harvestDistribution/distributionAnalisis/output/finalFigures/mapHarvest_porFarm.png", height = 14, width = 12.5, device = "png")
ggsave(FIG_MAP_GAMMA_SAM_GRAY, filename= "../output/figurasDefensa/sam_mapHarvest_porFarm_gray.png", height = 9, width = 9, device = "png")
...
#DF_TOTAL_MIN <- read.csv("archivosTrabajandose/harvestDistribution/distributionAnalisis/output/dataFramesGenerated/DF_TOTAL_AIC_porFincas.csv")
...
DIS_PLOT_O<- DF_HARVEST_GAMMA %>%
  ...
  theme(legend.position = "none")+
  theme(strip.background =element_rect(fill="white"))+
  labs(x= "Longitud de paso (m)", y= "Frequencia", col= "Estado")
...
DIS_PLOT_C<- DF_HARVEST_GAMMA %>%
  ...
  theme(legend.position = "none")+
  theme(strip.background =element_rect(fill="white"))+
  labs(x= "Longitud de paso (m)", y= "Frecuencia", col= "Estado")
...
ggsave(DIS_PLOT_TOTAL, filename= "../output/figurasDefensa/histoGAMMA_porPlantacion.png", height = 6, width = 14, device = "png")


######################################################################3
#figuras vacias

DIS_PLOT_O_VAC<- DF_DISTRI_O %>%
  ggplot(aes(x= X, y= PDF, col= as.factor(state))) +
  scale_color_manual(values = 
                       mycolsStates)+
  facet_wrap(~farm)+
  ylim(0, 0.32)+
...
DIS_PLOT_VAC_TOTAL <- DIS_PLOT_C_VAC + DIS_PLOT_O_VAC

ggsave(DIS_PLOT_VAC_TOTAL, filename= "../output/figurasDefensa/histoGAMMA_porPlantacion_VAC.png", height = 6, width = 14, device = "png")
...
HIST_ST1 <- DF_HARVEST_TOTAL %>%
  ...
  labs(x= "Plantación", y= "Numero de cuadros visitados", fill= "Plantación", title = "Lado del cuadro (m)")

ggsave(HIST_ST1, filename= "../output/figurasDefensa/figStates_porFinca_porGrano.png", height = 6, width = 12, device = "png")


write_csv(DF_HARVEST_TOTAL, file = "../data/areaPorGrano.csv")

####boc vaci 
HIST_ST1_VAC <- DF_HARVEST_TOTAL %>%
  ggplot(col= "black", aes(x= farm, y= conteoArea, 
  ))+
...
ggsave(HIST_ST1_VAC, filename= "../output/figurasDefensa/figStates_porFinca_porGrano_VAC.png", height = 6, width = 12, device = "png")
