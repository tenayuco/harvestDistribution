...
#DF_1000_SIN$Y_st1_par0 <- DF_1000_SIN$st1_par0
```
...
####AQUI PASAMOS LOS DATOS A MEANS PARA WEIBULL
...
Some combinations resulted in distributions with zero variance. We decided to remove those cases as they did not make any biological sense. We then plotted the minimum negative log of the likelihood without these outliers (Fig.4). The minimum negative log-likelihood is equivalent to the maximum likelihood [@zucchini2016hidden]. Almost all the combinations of prior parameters converge to two models with equivalent minimum likelihood for both distributions (Fig. 4). In this sense, the parameters of the models with minimal likelihoods are robust to the initial parameters. 
...
We also explored the differences in the estimated parameters between the resulting states and their AIC. This corroborates that the models with the minimal AIC truly represented two-states models for both distributions, and have a lower AIC than one state models (Fig. 5). For each family of distributions we picked the model that had the minimum AIC value (table 3 and one of the purple dots in Fig. 5). 


```{r, fig.cap= "Parameters space for both states for each of the distributions. The (x,y) coordinate of each dot represent the mean and the horizontal and vertical errorbar the standard deviation, of state 1 and state 2, respectively. The dotted line represent the identity, where both states are equivalent. The color represent the AIC value, where gray values show infinitely big values. The purple dots represent the minimal AIC", echo=FALSE, fig.height= 4, fig.width=9}
