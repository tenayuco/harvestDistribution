...
Here we will use the _movehmm_ library (ref*) to i) fit the most likely HMM to the harvester movement data and ii) describe the different states, the sequence of states and the difference between farms. 

### 1. Fitting of HMM

#### 1.1. Preparation of data

We loaded _ggplot2_, _dplyr_, _tidyverse_ and _movehmm_ libraries. We then loaded the data of the trajectories and did some punctual modifications to it. We have in total 12 trajectories, with ($x, y$) coordinates:
...
The fitting algorithm (ref*) of the _movehmm_ library uses:

a) a predefined number of states. Here we will use two. 

b) a defined family of distributions (_gamma_, _lnorm_, or _weibull_). Here we will try to fit each of the families.
...
```{r, fig.show='hold', out.width="50%", echo=FALSE, fig.cap="lnorm model"}
