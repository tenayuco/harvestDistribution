...
## I. Short introduction on HMM 

Hidden Markov models (HMMs) are models in which the distribution that generates an observation {Z_t} depends on the state {S_t} of an underlying and unobserved Markov process (that satisfies the Markov property) () (ref*, fig.). In the context of animal movement, the state {S_t} is interpreted as a proxy for the behavioral state of the animal (foraging, exploring). The observations {Zt} are bivariate time series Zt = (lt, phi_t) where lt is the step length (the Euclidean distance) and ot the turning angle, between two succesive locations. Biologically speaking, these models try to include the process where the movement of agents (e.g. short or large steps) depends on its behavior. With these models, we can define the most likely sequence of states along the trajectory of an agent, its average time within a state, and the number of switches between states.  
...
### I.1. Preparation of data

We load ggplot2, dplyr, tidyverse and movehmm libraries used during the analysis. We then load the data of the trajectories and do some punctual modifications to it. We have in total 12 trajectories, with X, y coordinates:
...
```{r echo=FALSE, warning=FALSE, fig.cap= "12 trajectories of harvesters. The green line represent the Finca Hamburgo (H) and the blue line the Finca Irlanda (I)" , fig.height= 12, fig.width=10}
...
For each combination we registered the minimum negative-likelihood and plot it: 
...
Some results registered combinations where one of the distributions had zero variance.We decided to remove those cases has they did not make biological sense. 
...
head(DF_1000_OUTLIERS)
```

We then plot the min negative log of the likelihood without these outliers 
...
We take for each family of distributions the minimini likeloojd 

FALTA DESPUES DECIR CUAL SE TOAM POR EL CRITERIP DE AIC!°!!
```{r}
